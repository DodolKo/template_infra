...
docker exec -i melchior psql -U "$PG_USER" -d "$PG_DB" -c "CREATE TABLE IF NOT EXISTS test_sync (id SERIAL PRIMARY KEY, val TEXT, created_at TIMESTAMP DEFAULT NOW());" > /dev/null
docker exec -i melchior psql -U "$PG_USER" -d "$PG_DB" -c "INSERT INTO test_sync (val) VALUES ('$TEST_KEY');" > /dev/null

echo "2. Waiting 1 second for WAL replication..."
sleep 1

echo "3. Verifying record presence on Replica 1 (balthasar)..."
BALTHASAR_COUNT=$(docker exec -i balthasar psql -U "$PG_USER" -d "$PG_DB" -t -c "SELECT COUNT(*) FROM test_sync WHERE val='$TEST_KEY';" | tr -d ' ')

echo "4. Verifying record presence on Replica 2 (casper)..."
CASPER_COUNT=$(docker exec -i casper psql -U "$PG_USER" -d "$PG_DB" -t -c "SELECT COUNT(*) FROM test_sync WHERE val='$TEST_KEY';" | tr -d ' ')
